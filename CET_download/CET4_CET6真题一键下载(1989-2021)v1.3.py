...
class Runthread(QThread):
    ...
    def get(self,url, count, presentPath):
        ...
        liList = tree.xpath('/html/body/div[1]/div[1]/div[1]/ul/li')
        temp = int(0)
        n = int(1)
        for li in liList:
            if temp < n:
                temp = temp + 1
                continue
            judge = li.xpath('./div/div[1]/a[2]')  # 如果li标签下的div/div[1]/a[2]存在，说明是可以直接下载的文件
            if not judge:  # 目录文件
                fileName = li.xpath('./div/@data-title')[0].strip(' ')#python中的strip()可以去除头尾指定字符
                ...
                downLoadName = li.xpath('./div/@data-title')[0].strip(' ')
...
if __name__ == "__main__":
    ...
    #设置标题
    myWin.setWindowTitle('CET4_CET6真题一键下载(1989-2021)V1.3')
    # 将窗口控件显示在屏幕上
    myWin.show()
